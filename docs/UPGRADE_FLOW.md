...
```
solver 发现需要升级 → 创建 upgrader 子任务 → upgrader 修改主仓 → 提交 → 恢复 solver
```
...
| `upgrader` | 升级子任务 | 可写主仓 | `new` → `running` → `done` |

### 交互流程

```
1. Solver 运行 → 发现需要升级 (status="needs_upgrade")
2. Scheduler 创建 upgrader 子任务 (priority=99, parent_tid=<solver_tid>)
3. Solver 状态设为 `upgrade-waiting`
4. Upgrader 运行 → 修改主仓代码
5. Upgrader 提交 → 状态 `done`
6. Scheduler 恢复父任务 → solver 继续运行
```
...
Solver:
  ...
  new → running → upgrade-waiting → (after upgrader done) → running → done
...
task/<solver_tid>/
  ...
  parent_tid     ← (for upgrader) points to solver
...
1. **并行升级**：多个 upgrader 并行（优先级控制）
2. **升级验证**：upgrader 运行 smoke check
3. **回滚机制**：升级失败自动回滚
